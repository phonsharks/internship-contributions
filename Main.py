...
def insert_into_database(cursor, cnxn, row, error_m, flag):
    ...
    cursor.execute(
        f'''IF NOT EXISTS (SELECT * FROM zt_trendyol_supplier_invoice_links WHERE OrderNumber='{row.OrderNumber}')
            INSERT INTO zt_trendyol_supplier_invoice_links (Flag, OrderNumber, DocumentNumber, Description, shipmentPackageId, invoiceLink, OfficeCode, InvoiceHeaderID, LastUpdatedTime)
            VALUES ({flag}, '{row.OrderNumber}', '{row.DocumentNumber}', '{error_m}', '{row.shipmentPackageId}', '{row.invoiceLink}', '{row.OfficeCode}', '{row.InvoiceHeaderID}', GETDATE())
        ELSE IF (SELECT Flag FROM zt_trendyol_supplier_invoice_links WHERE OrderNumber='{row.OrderNumber}') != 1
            ...
            SET Flag = {flag}, Description = '{error_m}', invoiceLink = '{row.invoiceLink}',InvoiceHeaderID = '{row.InvoiceHeaderID}', LastUpdatedTime = GETDATE()
